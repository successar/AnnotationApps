...
def basic_annotator(get_dataset, display, selector_elements=None):
    state = SessionState.get(
        username=secrets.token_hex(16), key=secrets.token_hex(128), assignments={}, get_next=True
    )
    ...
    with st.spinner("Loading document"):
        annotations = annotation_block(
            page=display(
                dataset[selected_assignment], username=username, assignment=selected_assignment, key=state.key
            ),
            key=state.key,
        )
